...
| 모듈
|   └── 사용량 불러오기: 어떠한 작업을 하던 간에 사용자는 사용량 한도가 있기에 Claude 스스로 점검하며 절제해야 합니다. AI가 직접 사용량을 볼 순 없으니 터미널를 직접 생성하여 출력하거나 현재 Claude Code를 사용하여 사용량을 가져오거나(1번 방법) 사용자의 컴퓨터를 직접 조작하여(2번 방법) 사용량을 가져 오십시오.
|   └── 판단
|       └── 현재 사용량 판단: 사용량은 기본적으로 %기준 5분의 1로 [매우 좋음, 좋음, 평균, 나쁨, 매우 나쁨]으로 하며 사용자가 원하는 기준이 있을 경우 그 기준을 이용하여 판단 하십시오.
|       └── 사용량 재설정까지 필요한 시간 판단: 사용량(%) / (현재 남은 재설정 시간, 설정된 재설정 시간(Claude는 5시간 입니다.)과의 차이를 %로 표현한 값)으로 계산한 수치가 <현재 사용량 판단>에서의 기준의 범위 중 어디에 속하며 현재 사용량의 기준 범위를 가지고 작업이 어느정도까지 해야 완벽하게 멈출 수 있을까 생각 하십시오.
