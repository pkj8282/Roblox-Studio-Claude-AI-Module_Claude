...
- [ ] 사용자의 요청이 게임 설정에서 활성화 해야하는 서비스가 필요할 때 서비스를 미리 호출 함으로써 서비스가 호출 가능한가?
...
#### 서버에서 발생한 상호작용 확인
1. **엑세스에서 발생한 변화에 대해 서버에 확인 요청**: <@엑세스> 목록에서 클라이언트가 접근 가능한 엑세스를 이용하여 클라이언트 내에서 특정 객체를 바꾸거나 상호작용 한 것을 감지했을 때 서버에 의한 것인지 확인 요청합니다.
2. **서버의 응답 대기**: 서버가 확인 요청에 대해 응답할 때까지 대기하며 응답 완료시 서버에 의한 것이 아니라면 서버에 감독 요청을 합니다.
...
4. **데이터 분석에 대한 대응**: 분석 결과에서 다르다는 것이 확인되면 해당 클라이언트의 연결을 해제합니다. 분석 결과가 같다면 **최대 3회까지** 3분 간격으로 이 절차를 반복합니다.

### 보안 체크리스트
- [] 민감한 데이터는 <@객체 관리>/서버 감독 공유 사이드/ 혹은 <@객체 관리>/서버 사이드/로 설정
- [] 클라이언트의 요청에서 보안적인 밀접 관계가 있는 요청은 적절한 RLS 정책을 설정
- [] 서버와 클라이언트의 상호작용이 서버와 밀접한 경우 중요한 상호작용으로 서버가 우선적으로 활동

### lua 코드 작성 시 규칙
**백업**: 코드를 편집하는 것이라면 편집 하기 전에 미리 코드 혹은 해당 코드의 최상위 모델 및 폴더(기본적인 서비스 디렉토리 제외)로부터 복제하여 <game.ServerStorage> 에 저장합니다. <@백업용 저장소 관리>에 따라 백업용 저장소 사용 플래그가 false라면 백업을 하지 않습니다. **백업본 이름 저장 방식**: 해당 코드의 이름_해당 코드의 상위부모 객체 이름_해당 코드의 서비스 디렉토리 이름_타임스탬프 / 해당 코드의 이름_해당 코드의 상위부모 객체 이름_타임스탬프 (상위부모가 서비스 객체와 같을 때만)
**사용자의 검증**: 모든 코드는 작성을 마치면 사용자에게 코드를 보여주고 사용자가 거절하거나 허용 및 제안 할 수 있도록 대기 합니다.
**최적화**: 모든 코드는 최적화 절차를 통해 완료되어야 합니다.
**불필요한 인덱싱**: 필요하지 않은 요소를 인덱싱 하지 않습니다.
...
|── 사용자가 지정한 코드에서 분석한 결과가 사용자의 요청과 전혀 다른 결과일 때: 만약 사용자가 필요 시 추가 분석 스크립트 위치를 알려주었다면 위치를 참고하여 해당 스크립트를 추가 분석
...
|   └── 여러 작업
|       └── 처리 과정: 사용자가 여러 작업을 요청했다. 사용자가 우선순위를 주었다면 우선순위대로 작업을 시행하고 아니라면 각 작업에 대해 프레임 워크를 만들고 적은 순서로 작업하는 게 좋을 것이다.(사용자가 우선순위를 주었는지 확인하고 아니라면 각 작업에 대해 프레임 워크를 생성하고 프레임 워크의 요소가 적은 순서로 작업을 시행한다) 사용자에게 효율적인 점검을 제공해야 된다. 작업을 분류하여 최종 응답에 기록 내용을 제공 해야한다.(최종 응답 시퀀스가 시작되었을 때 완료된 작업과 완료되지 않은 작업으로 작업 내용을 분류하고 각 작업에 대해 발생한 상호작용들에 대해 작업 기록에 기록하여 최종 응답에 작업 기록을 제공한다)
| 문제 해결
|   └── 문제의 핵심 파악
...
# 자기 상태 점검
**Claude Code에서 현재 사용자의 AI 사용량 한도나 사용량 재설정 시간, 사용자가 요청한 중단 시간에 대하여 Claude가 스스로 점검할 수 있도록 돕는 체계 입니다.**
Claude는 사용자의 한도를 스스로 보는 체계가 없어 외부 체계를 이용해야 합니다. CLAUDE.md 파일의 상위 폴더에 상속되있는 claude-usage-tutorial.md 파일의 절차를 사용하여 사용자의 구독제에 따른 사용량을 가져와 아래 범위 기준에 따라 판단 하세요.
| 기준 표: 100%중 5개의 범위(x <= 20%(판단 필요 없음), 20%< x <= 40%(작업 하는데 예상되는 비용의 추상적인 판단 하에 조정), 40%< x <= 60%(사용량을 주기적으로 검토하면서 작업을 조심히 수행), 60%< x <= 80%(비용이 적게 들 것이라고 생각하는 부분부터 작업 수행), 80%< x <= 100%(현재 까지의 작업을 전부 보고서로 만든 다음(이미 있는 경우 갱신) 사용자에게 작업을 더 할 지부터 물어보기))

# 사용자의 컴퓨터 자율 제어
**Claude Code가 사용자의 컴퓨터를 자율적으로 제어하는데 대한 안정적인 제어 성격을 만드는 체계 입니다.**
| 사용자의 컴퓨터 제어 시 생각하는 절차: 사용자의 요청의 복잡도를 먼저 생각 -> 생각한 복잡도를 통해 해당 요청이 사용자의 컴퓨터에 피해가 갈 만한 확률이 커보이는 지 생각 -> 생각한 복잡도와 피해 확률을 통해서 바로 할 만한지 판단 후 실행
| 컴퓨터 종료, 예약 부팅 시 강제 한계: RTC Boot가 지원되지 않는 경우가 있으니 S4(최대 절전 모드)를 활용 하세요.
